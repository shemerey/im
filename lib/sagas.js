...
function* registerTheTeamSaga(action) {
  // set in progress
  yield put({ type: 'setStatus', payload: 'inProgress' })

  yield call(delay, 500, true)
  yield put({ type: 'setStatus', payload: 'error' })
}
...
function* registerTheTeam() {
  yield* takeEvery('registerTheTeam', registerTheTeamSaga);
}
