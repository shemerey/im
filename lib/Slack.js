...
import { updateTeam, addNewUser, addNewChannel } from './actions'
...
export default class Slack {
  ...
  fullFillUsers(users) {
    users.forEach((user) => {
      ...
      store.dispatch(addNewUser(userObject))
    })
  }

  fullFillChannels(channels) {
    channels.forEach((channel) => {
      const channelObject = new ChannelObject({
        id: channel.id,
        teamId: this.id,
        name: channel.name,
        type: 'group',
      })
      store.dispatch(addNewChannel(channelObject))
    })
  }
