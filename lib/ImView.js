...
export default class ImView {
  constructor({ store, providerFactory, imChannelSelectView, serializedState: { imViewState } }) {
  ...
  show() {
    ...
    if (!this.activated) {
      setTimeout(() => {
        ...
        this.providerFactory.perform().then(() => {
          this.store.dispatch(imIsReadyAction())
          this.store.dispatch(imHasNoProblemsAction())
        }).catch((err) => {
          this.store.dispatch(imHasProblemsAction(err))
        })
      }, 0)
